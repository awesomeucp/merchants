...
export function MerchantCard({ merchant }: MerchantCardProps) {
  ...
  const paymentProviders = merchant.ucpProfile.paymentHandlers
    ...
    .filter((value, index, self) => self.indexOf(value) === index); // Unique providers
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-500">
            <span className="flex items-center gap-1">
              <Lightning size={14} weight="fill" className="text-blue-600 dark:text-blue-400" />
              {capabilityCount} capabilities
            </span>
            <span className="flex items-center gap-1">
              <CreditCard size={14} weight="regular" className="text-blue-600 dark:text-blue-400" />
              {paymentProviders.length} payment methods
            </span>
          </div>
        </div>
      </Card>
